...
use crate::{AppWindow, SettingsState, AppState};
        ...
        SettingsState::get(ui).on_save_config(move |server_addr, server_port, webhook_addr, webhook_port| {
            ...
            rt_handle.spawn(async move {
                let _ = tx.send(vouchflow::domain::DbCommand::SaveConfig {
                    key: "server_addr".into(), value: server_addr, category: "NETWORK".into()
                }).await;
                let _ = tx.send(vouchflow::domain::DbCommand::SaveConfig {
                    key: "server_port".into(), value: server_port, category: "NETWORK".into()
                }).await;
                let _ = tx.send(vouchflow::domain::DbCommand::SaveConfig {
                    key: "webhook_addr".into(), value: webhook_addr, category: "NETWORK".into()
                }).await;
                let _ = tx.send(vouchflow::domain::DbCommand::SaveConfig {
                    key: "webhook_port".into(), value: webhook_port, category: "NETWORK".into()
                }).await;
                ...
                let _ = slint::invoke_from_event_loop(move || {
                    if let Some(ui) = ui_weak.upgrade() {
                        ...
                        let _ = slint::Timer::single_shot(std::time::Duration::from_secs(2), move || {

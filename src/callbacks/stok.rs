...
pub fn register(
            ...
            let (provider, search) = {
                if let Some(ui) = ui_weak.upgrade() {
                    ...
                    let prov = if idx > 0 && (idx as usize) < providers.iter().count() {
                        providers.row_data(idx as usize).unwrap_or("Semua".into()).to_string()
                    } else {
                        "Semua".to_string()
                    };
                    ...
                    Ok(stocks) => {
                        ...
                        let _ = slint::invoke_from_event_loop(move || {
                            if let Some(ui) = ui_weak.upgrade() {
                                ...
                                let rows: Vec<slint::ModelRc<slint::StandardListViewItem>> = stocks.iter().map(|s| {
                                    let items: Vec<slint::StandardListViewItem> = vec![
                                        slint::StandardListViewItem::from(s.provider.as_str()),
                                        slint::StandardListViewItem::from(s.kode_addon.as_str()),
                                        slint::StandardListViewItem::from(s.barcode.as_str()),
                                        slint::StandardListViewItem::from(s.serial_number.as_str()),
                                        slint::StandardListViewItem::from(format_date(&s.expired_date).as_str()),
                                        slint::StandardListViewItem::from(s.status.as_str()),
                                        slint::StandardListViewItem::from(format_date(&s.created_at).as_str()),
                                    ];
    ...
    // --- Load used stocks ---
    {
        ...
        StokState::get(ui).on_load_used_stocks(move |date_from, date_to| {
            ...
            let ui_weak = ui_handle.clone();
            let db = db_ref.clone();
            let from = if date_from.is_empty() { None } else { Some(date_from.to_string()) };
            let to = if date_to.is_empty() { None } else { Some(date_to.to_string()) };
            let (provider, search) = {
                if let Some(ui) = ui_weak.upgrade() {
                    let state = StokState::get(&ui);
                    let idx = state.get_provider_filter_index();
            ...
            rth.spawn(async move {
                let search_opt = if search.is_empty() { None } else { Some(search.as_str()) };
                match services::get_used_stocks(&db, Some(provider.as_str()), from.as_deref(), to.as_deref(), search_opt).await {
                    Ok(stocks) => {
    ...
    // --- Save stock ---
    {
        ...
        let rth = rt_handle.clone();
        StokState::get(ui).on_save_stock(move |id, provider, kode_addon, barcode, serial, expired| {
                ...
                match services::get_stock_by_id(&db, id as i64).await {
                    Ok(Some(stock)) => {
                        let provider_idx = match stock.provider.as_str() {
                            "Byu" => 0, "Smartfren" => 1, "Telkomsel" => 2, _ => 0,
                        };
                        let _ = slint::invoke_from_event_loop(move || {
                            if let Some(ui) = ui_weak.upgrade() {
                                let state = StokState::get(&ui);
            ...
            rth.spawn(async move {
                match services::get_stock_addon_summary(&db).await {
                    Ok(summary) => {
                        ...
                        let _ = slint::invoke_from_event_loop(move || {
                            if let Some(ui) = ui_weak.upgrade() {
                                let rows: Vec<slint::ModelRc<slint::StandardListViewItem>> = summary.iter().map(|s| {
                                    let items: Vec<slint::StandardListViewItem> = vec![
                                        slint::StandardListViewItem::from(s.kode_addon.as_str()),
                                        slint::StandardListViewItem::from(format!("{}", s.count).as_str()),
                                    ];
                                    slint::ModelRc::new(slint::VecModel::from(items))
                                }).collect();
                                StokState::get(&ui).set_stock_summary(slint::ModelRc::new(slint::VecModel::from(rows)));
                            }
                        });
                    }
    ...
    // --- Range-based delete ---
    {
        ...
        StokState::get(ui).on_delete_stocks_by_range(move |start, end, all_ids| {
            ...
            tracing::info!("Deleting stocks by range {}-{} ({} items): {:?}", start, end, ids.len(), ids);
            if !ids.is_empty() {
    ...
    // --- Check voucher ---
    {
        ...
        StokState::get(ui).on_check_voucher(move |barcode, provider| {
            ...
            tracing::info!("Checking voucher: barcode={}, provider={}", barcode, provider);
            ...
            rth.spawn(async move {
                let result = match provider.as_str() {
                    ...
                    "Smartfren" => { let p = SmartfrenProvider::new(); p.check_voucher(&barcode).await }
                ...
                let _ = slint::invoke_from_event_loop(move || {
                    if let Some(ui) = ui_weak.upgrade() {
                        ...
                        match result {
                            Ok(resp) => {
                                state.set_check_result_product(resp.product_name.unwrap_or_default().into());
                                let expiry_date = resp.expiry_date.clone().unwrap_or_default();
                                let date_only = expiry_date.split(|c| c == ' ' || c == 'T').next().unwrap_or(&expiry_date);
                        ...
                        countdown_rt.spawn(async move {
                            for remaining in (0..5).rev() {
                                ...
                                if stok_check_dialog_session_task.load(std::sync::atomic::Ordering::SeqCst) != session_id { break; }
                                let ui_weak_tick = ui_weak_countdown.clone();
                                let stok_check_dialog_session_tick = stok_check_dialog_session_task.clone();
                                let _ = slint::invoke_from_event_loop(move || {
                                    if stok_check_dialog_session_tick.load(std::sync::atomic::Ordering::SeqCst) != session_id { return; }
                                    if let Some(ui) = ui_weak_tick.upgrade() {
                                        let state = StokState::get(&ui);
                                        if !state.get_check_dialog_open() || state.get_is_checking() { return; }
                                        state.set_check_dialog_countdown(remaining);
                                        if remaining == 0 { state.set_check_dialog_open(false); }
                                    }
                                });
    ...
    // --- Check expired ---
    {
        ...
        StokState::get(ui).on_check_expired(move |provider, barcode| {
            ...
            let provider = provider.to_string();
            let barcode = barcode.to_string();
            tracing::info!("Checking expired: provider={}, barcode={}", provider, barcode);
            let ui_weak = ui_handle.clone();
            ...
            rth.spawn(async move {
                let result = match provider.as_str() {
                    ...
                    "Smartfren" => { let p = SmartfrenProvider::new(); p.check_voucher(&barcode).await }
                    _ => Err(vouchflow::application::providers::ProviderError::UnknownProvider(
                        format!("Provider {} tidak mendukung cek expired", provider)
                    )),
                };
                let _ = slint::invoke_from_event_loop(move || {
                    if let Some(ui) = ui_weak.upgrade() {
                        match result {
                            Ok(resp) => {
                                if let Some(expiry) = resp.expiry_date {
                                    let date_only = expiry.split(|c| c == ' ' || c == 'T').next().unwrap_or(&expiry);

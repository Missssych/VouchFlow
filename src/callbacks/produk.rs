...
pub fn register(
                        ...
                        let _ = slint::invoke_from_event_loop(move || {
                            if let Some(ui) = ui_weak.upgrade() {
                                    ...
                                    products
                                        ...
                                        .map(|p| {
                                            let items: Vec<slint::StandardListViewItem> = vec![
                                                ...
                                                slint::StandardListViewItem::from(
                                                    p.nama_produk.as_str(),
                                                ),
                                                slint::StandardListViewItem::from(
                                                    p.kode_produk.as_str(),
                                                ),
                                                slint::StandardListViewItem::from(
                                                    p.kategori.as_str(),
                                                ),
                                ...
                                let (rows_model, ids_model) =
                                    with_table(TableId::Produk, |m| m.set_all(rows, ids));
    ...
    // --- Search products ---
    {
        ...
        ProdukState::get(ui).on_search_products(move |query, provider, kategori| {
            ...
            let k = if kategori.is_empty() {
                None
            } else {
                Some(kategori.to_string())
            };
            rth.spawn(async move {
                let provider_filter = p.as_deref();
                let kategori_filter = k.as_deref();
                match svc
                    .search_products(&q, provider_filter, kategori_filter)
                    .await
                {
                    Ok(products) => {
                        let _ = slint::invoke_from_event_loop(move || {
    ...
    // --- Save product ---
    {
        ...
        let rth = rt_handle.clone();
        ProdukState::get(ui).on_save_product(
            move |id, provider, nama, kode_produk, kategori, harga, addon| {
                let ui_weak = ui_handle.clone();
                let cmd_tx = tx.clone();
                ...
                let addon_opt = if addon.is_empty() {
                    None
                } else {
    ...
    // --- Delete products by range ---
    {
        ...
        ProdukState::get(ui).on_delete_products_by_range(move |start, end, all_ids| {
            ...
            tracing::info!(
                "Deleting products by range {}-{} ({} items)",
                start,
                end,
                ids.len()
            );
                    ...
                    Ok(Some(product)) => {
                        ...
                        let _ = slint::invoke_from_event_loop(move || {
                            if let Some(ui) = ui_weak.upgrade() {
                                ...
                                let addon_idx = addon_options
                                    .iter()
                                    .position(|a| a.as_str() == addon_val.as_str())
                                    .map(|i| i as i32)
                                    .unwrap_or(-1);
                    ...
                    Ok(Some(product)) => {
                        ...
                        let _ = slint::invoke_from_event_loop(move || {
                            if let Some(ui) = ui_weak.upgrade() {
                                ...
                                state.set_form_provider_index(provider_idx);
                                state.set_form_nama_produk(
                                    format!("{} (Copy)", product.nama_produk).into(),
                                );
                                state.set_form_kode_produk(
                                    format!("{}_COPY", product.kode_produk).into(),
                                );
                                state.set_form_kategori_index(kategori_idx);
                                state.set_form_harga(format!("{:.0}", product.harga).into());
                                let addon_val = product.kode_addon.unwrap_or_default();
            ...
            rth.spawn(async move {
                match svc.get_addon_options().await {
                    Ok(addons) => {
                        let _ = slint::invoke_from_event_loop(move || {
                            if let Some(ui) = ui_weak.upgrade() {
                                let addon_model: Vec<slint::SharedString> = addons
                                    .iter()
                                    .map(|a| slint::SharedString::from(a.as_str()))
                                    .collect();
                                ProdukState::get(&ui).set_addon_options(slint::ModelRc::new(
                                    slint::VecModel::from(addon_model),
                                ));
                            }
                        });
                    }

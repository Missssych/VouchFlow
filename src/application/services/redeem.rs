...
pub async fn execute_redeem(
    ...
    if !circuit_breaker.is_allowed().await {
        append_flow_log(
            ...
            "WARN",
            Some("FAILED"),
            "Circuit breaker rejected redeem request",
            None,
            None,
        )
        .await;
    ...
    append_flow_log(
        ...
        "INFO",
        Some("PROCESSING"),
        "Reserving voucher from stock via DB Writer",
        None,
        None,
    )
    .await;
    ...
    if let Err(e) = db_cmd_tx
        .send(DbCommand::ReserveStokVoucher {
        ...
        .await
    {
        append_flow_log(
            ...
            "ERROR",
            Some("FAILED"),
            "Failed to send reserve command",
            Some(e.to_string()),
            None,
        )
        .await;
    ...
    match provider_router
        .redeem_voucher(provider_name, nomor, &voucher.serial_number)
        .await
    {
        Ok(response) => {
            ...
            if response.success {
                ...
                append_flow_log(
                    ...
                    "INFO",
                    Some("SUCCESS"),
                    "Redeem flow finished successfully",
                    None,
                    None,
                )
                .await;
                ...
                let _ = db_cmd_tx
                    .send(DbCommand::ReleaseStokVoucher {
                        voucher_id: voucher.voucher_id,
                    })
                    .await;
                ...
                append_flow_log(
                    ...
                    "WARN",
                    Some("FAILED"),
                    "Redeem flow finished with business failure",
                    None,
                    None,
                )
                .await;
        ...
        Err(e) => {
            ...
            append_flow_log(
                ...
                "ERROR",
                Some("FAILED"),
                "Redeem flow finished with provider error",
                None,
                None,
            )
            .await;

...
use crate::domain::{
    ConfigEntry, DomainError, DomainEvent, LogEntry, MonitoringCounters, TransactionSummary,
};
...
impl CentralStore {
    ...
    pub async fn hydrate_from_db(&self, db: &Database) -> Result<(), DomainError> {
        ...
        db.with_reader(|conn| {
            let (total, success, failed, pending): (i64, i64, i64, i64) = conn
                .query_row(
                    "SELECT 
                    COUNT(*),
        ...
        db.with_reader(|conn| {
            ...
            let configs: Vec<ConfigEntry> = stmt
                .query_map([], |row| {
        ...
        db.with_reader(|conn| {
            ...
            let mut logs: Vec<LogEntry> = stmt
                .query_map([self.log_buffer_size as i64], |row| {

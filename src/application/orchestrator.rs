...
use crate::domain::{Command, DbCommand, DomainError, TransactionResult, TransactionType};
use crate::infrastructure::channels::{CommandReceiver, DbCommandSender};
use crate::infrastructure::database::Database;
use crate::infrastructure::provider::{CircuitBreaker, CircuitBreakerConfig, ProviderClient};
use std::sync::Arc;
...
impl Orchestrator {
    ...
    pub async fn run(self) {
        ...
        loop {
            let cmd_opt = {
                let mut rx: tokio::sync::MutexGuard<'_, CommandReceiver> =
                    orchestrator.command_rx.lock().await;
                rx.recv().await
            };
    ...
    async fn process_command(&self, cmd: Command) -> Result<(), DomainError> {
        ...
        if is_retry {
            ...
            self.db_cmd_tx
                .send(crate::domain::DbCommand::UpdateTransaction {
                    tx_id: tx_id.clone(),
            ...
            self.db_cmd_tx
                .send(crate::domain::DbCommand::InsertTransaction {
                    tx_id: tx_id.clone(),
    ...
    async fn is_duplicate_request(&self, request_id: &str) -> Result<bool, DomainError> {
        self.db
            .with_reader(|conn: &rusqlite::Connection| {
                let count: i64 = conn
                    .query_row(
    ...
    async fn append_flow_log(
        ...
        self.db_cmd_tx
            .send(crate::domain::DbCommand::AppendTransactionLog {
